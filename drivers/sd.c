...
int
sd_cmd(unsigned int code, unsigned int arg)
{
    int r = 0;
    sd_err = SD_OK;
    ...
    *EMMC_INTERRUPT=*EMMC_INTERRUPT; *EMMC_ARG1=arg; *EMMC_CMDTM=code;
    ...
    if ((r = sd_int(INT_CMD_DONE))) { uart_puts("ERROR: failed to send EMMC command"); sd_err=r; return 0; }
    r = *EMMC_RESP0;
             ...
             unsigned int num)
{
    ...
    while (c < num) {
        ...
        if ((r = sd_int(INT_READ_RDY))) { uart_puts("\r ERROR: Timeout waiting for ready to read\n"); sd_err = r; return 0; }
...
sd_clk(unsigned int f)
{
    ...
    *EMMC_CONTROL1 &= ~C1_CLK_EN;
    ...
    d = (((d & 0x0ff) << 8) | h);
    *EMMC_CONTROL1 = (*EMMC_CONTROL1 & C1_CLK_STABLE) | d;
    wait_msec(10);
    *EMMC_CONTROL1 |= C1_CLK_EN;
    wait_msec(10);
